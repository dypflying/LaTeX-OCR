...
def inference_easylatex(source_data, model_path:str):
    from pix2tex.inference import LatexOCR
    model = LatexOCR(model_path)

    images = load_images(source_data)
    write_data = []
    for i in tqdm(range(len(images)), desc="Easylatex inference"):
        try:
            text = model(images[i])
        except ValueError:
...
def main():
    ...
    if args.easylatex != "":
        start = time.time()
        predictions = inference_easylatex(source_data, args.easylatex)
        times["easylatex"] = time.time() - start

        p_text = [normalize_text(p["text"]) for p in predictions]

        p_scores = score_text(p_text, references)

        write_data["easylatex"] = {
            "scores": p_scores,
